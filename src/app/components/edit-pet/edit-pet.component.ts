...
export class EditPetComponent implements OnInit {
  ...
  updatePet() {
    this.petsService.Update(Number(this.petDetail.id), this.petDetail).subscribe({
      next: (params: any) => { this.router.navigate(['/Farm/PetByFarmIdFarmId/' + this.petsService.currentFarmId]); console.log(params); },
  ...
  deletePet(id: string) {
    ...
    this.petsService.Delete(Number(this.petDetail.id)).subscribe(
      {
        next: (params) => { this.router.navigate(['/Farm/PetByFarmIdFarmId/' + this.petsService.currentFarmId]) },
        error: (response: any) => { console.log(response); }
      }
    )
  }
